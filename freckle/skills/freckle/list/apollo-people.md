...
Available inline filters are repeatable `--title`, repeatable `--location`, repeatable `--excluded-title`, repeatable `--excluded-location`, repeatable `--seniority`, `--keyword`, repeatable `--company-domain`, repeatable `--company-location`, and `--strict-titles`. Apollo expands similar titles by default; `--strict-titles` disables that expansion. `--seniority` accepts `owner`, `founder`, `c_suite`, `partner`, `vp`, `head`, `director`, `manager`, `senior`, `intern`, and `entry`. `--limit` accepts 1–50,000 people and defaults to 100.
