...
- BYOK uses `mode: "byok"` with `provider`, `modelId`, and `credentialId` for an existing matching-provider workspace connection. Supported pairs: `openai` / `gpt-5.6-luna` or `anthropic` / `claude-sonnet-5`.
